...
#include "AbilitySYstem/Hero/Slot/ZodiacHeroAbilitySlotActor.h"
...
AZodiacHostCharacter* UZodiacHeroAbility::GetHostCharacterFromActorInfo() const
{
	return (CurrentActorInfo ? Cast<AZodiacHostCharacter>(CurrentActorInfo->OwnerActor.Get()) : nullptr);
}

UZodiacAbilitySystemComponent* UZodiacHeroAbility::GetHeroAbilitySystemComponentFromActorInfo() const
{
	if (UAbilitySystemComponent* ASC = GetAbilitySystemComponentFromActorInfo())
	{
		return Cast<UZodiacHeroAbilitySystemComponent>(ASC);
	}
...
{
	return (CurrentActorInfo ? Cast<AZodiacPlayerController>(GetHostCharacterFromActorInfo()->GetController()) : nullptr);
}
...
void UZodiacHeroAbility::SetCameraMode(TSubclassOf<UZodiacCameraMode> CameraMode)
{
	ENSURE_ABILITY_IS_INSTANTIATED_OR_RETURN(SetCameraMode, );

	if (AZodiacCharacter* ZodiacCharacter = GetHostCharacterFromActorInfo())
	{
...
void UZodiacHeroAbility::ClearCameraMode()
{
