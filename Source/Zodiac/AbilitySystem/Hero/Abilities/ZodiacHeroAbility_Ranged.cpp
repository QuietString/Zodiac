...
void UZodiacHeroAbility_Ranged::PerformLocalTargeting(TArray<FHitResult>& OutHits)
{
	...
	if (HeroActor && HostCharacter && HostCharacter->IsLocallyControlled() && WeaponSlot)
	{
...
FTransform UZodiacHeroAbility_Ranged::GetTargetingTransform(APawn* HostPawn, AActor* HeroActor, EZodiacAbilityAimTraceRule Source) const
{
	check(HostPawn);
	check(HeroActor);
	...
	const FVector ActorLoc = HeroActor->GetActorLocation();
	...
	if (Controller && ((Source == EZodiacAbilityAimTraceRule::CameraTowardsFocus) || (Source == EZodiacAbilityAimTraceRule::PawnTowardsFocus) || (Source == EZodiacAbilityAimTraceRule::WeaponTowardsFocus)) || (Source == EZodiacAbilityAimTraceRule::WeaponTowardsFocusHit))
	{
		...
		if (PC)
		{
			...
			{
				const FVector PawnLocation = HeroActor->GetActorLocation();
		...
		{
			CamLoc = HeroActor->GetActorLocation() + FVector(0, 0, HostPawn->BaseEyeHeight);
		}
	...
	if (UZodiacAbilitySystemComponent* HostASC = Cast<UZodiacAbilitySystemComponent>(GetHostAbilitySystemComponent()))
	{
		...
		if (ChargeUltimateEffectClass && FirstHitResult->bBlockingHit && HasAuthority(&CurrentActivationInfo))
		{
			if (AActor* HitActor = FirstHitResult->GetActor())
			{
				...
				if (TeamSubsystem->CanCauseDamage(GetHostCharacterFromActorInfo(), HitActor, false))
				{
